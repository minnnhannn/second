    ...
    public partial class home : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string qs1 = "SELECT distinct TOP 10 [MemberMaster].Account,[MemberMaster].Name,CASE WHEN NOT EXISTS(SELECT Own.Account WHERE Own.Account=[MemberMaster].Account) Then [MemberMaster].Balance WHEN EXISTS(SELECT Own.帳號 WHERE Own.Account=[MemberMaster].Account) THEN SUM(Stock.Closing_P*1000*Own.Amount)+[MemberMaster].Balance END AS'AccValue' FROM [User],Stock,Own  WHERE  ([MemberMaster].Capacity=@Capacity) AND (Stock.時間=@時間) AND (Own.Stock_Id=Stock.Stock_Id) AND [MemberMaster].Account=(case when EXISTS(select [MemberMaster].Name FROM Own WHERE [MemberMaster].Account=Own.Account) then (Own.Account) else ([User].Account) end ) group by [MemberMaster].Name,[MemberMaster].Balance,[MemberMaster].Account,[OWN].Account Order by AccValue DESC";
                //string qs1 = "SELECT Row_Number() over(order by AccValue DESC) as Rank,Account,AccValue,Ref_DT FROM Value";
                using (SqlConnection cn1 = new SqlConnection(cs1))
                {
                    cn1.Open();
                    using (SqlCommand command1 = new SqlCommand(qs1, cn1))
                    {
                        command1.Parameters.AddWithValue("@DataTime", Session["Date"]);
                        command1.Parameters.AddWithValue("@Capacity", "Member");
                        ...
                        if (dataset.Tables[0].Rows.Count != 0)   //判斷dataset有沒有值  (有值)
                        {
                            ...
                            {   //做十次 前十名
                                ...
                                string qs2 = "UPDATE Value SET AccValue=@AccValue WHERE (Account=@Account)";
                                //string qs2 = "UPDATE Rank SET Account = @Account,Name=@Name,AccValue=@AccValue WHERE (排名=@排名) ";
                                using (SqlConnection cn2 = new SqlConnection(cs2))
                                {
                                    cn2.Open();
                                    using (SqlCommand command2 = new SqlCommand(qs2, cn2))
                                    {
                                        command2.Parameters.AddWithValue("@Account", Convert.ToString(dataset.Tables[0].Rows[i][0]));
